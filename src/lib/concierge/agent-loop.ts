...
/** Catches the C2 hallucination pattern: a shopper naming a specific product/model, or asking about availability/comparison, before any tool has grounded the answer. */
const NEEDS_GROUNDING_PATTERN = /\b[A-Z][a-zA-Z]*\s?\d{1,3}\b|compare|\bvs\.?\b|versus|do you (have|carry|sell)|is there|in stock|available|discontinued|unreleased|not released/i;
...
export async function runConciergeTurn(
  ...
  const forceGroundingFirstPass = NEEDS_GROUNDING_PATTERN.test(latestUserMessage);
