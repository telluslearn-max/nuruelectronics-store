...
const explainBnplPlanDeclaration: FunctionDeclaration = {
  ...
  description:
    "Look up the real Buy Now, Pay Later terms for a product by its handle — deposit, installment amount, and eligibility requirements. Never state BNPL numbers or eligibility without calling this first. The result has both totalPayable (the installment-plan subtotal only — for the iPhone/iPad/MacBook formula-tier plans this excludes the deposit) and totalCost (the true all-in figure: deposit + every installment) — when the shopper asks what they'll pay in total or whether they can afford it, quote totalCost, not totalPayable. BNPL is live for iPhone, iPad, and MacBook only (weekly/monthly plans) — not other Apple-brand products like Apple Watch, AirPods, or Apple TV — and for Samsung/Google/OnePlus/Nothing (3-month/6-month plans, exact model+storage/RAM specific) — other brands are coming soon. If the result says needsVariantSelection, ask the shopper which storage/RAM they want and call again with variantId.",
  parametersJsonSchema: {
    type: "object",
    properties: {
