...
- Choosing a color scheme, typography system, spacing scale, or layout system
...
- The UI "doesn't look professional" but the reason isn't clear
- You've received usability or experience feedback
- You're doing a UI quality pass before launch
- You need to align design across platforms (Web / iOS / Android)
...
- Pure backend logic work
- API or database design only
- Non-visual performance work
...
*For human/AI reference: use priority 1→10 to decide which category to focus on first; use `--domain <Domain>` for details. The script itself does not read this table.*
...
- The default icon library is **Phosphor (`@phosphor-icons/react`)**. The entries listed in `src/ui-ux-pro-max/data/icons.csv` are common recommendations, not the full set.
- When no suitable icon is found in the recommendation table:
  - **Prefer picking any semantically closer icon from Phosphor's full icon set first**;
  - If Phosphor has no good option either, fall back to **Heroicons (`@heroicons/react`)**, keeping style consistent (line/filled, stroke weight, corner radius).
